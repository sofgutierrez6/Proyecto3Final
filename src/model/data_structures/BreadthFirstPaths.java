 ...
 *  @author Kevin Wayne
 */
public class BreadthFirstPaths<K extends Comparable<K>> {
    private static final int INFINITY = Integer.MAX_VALUE;
    private boolean[] marked;  // marked[v] = is there an s-v path
    private long[] edgeTo;      // edgeTo[v] = previous edge on shortest s-v path
     ...
     */
    public BreadthFirstPaths(Grafo G, K s) {
    ...
    private void bfs(Grafo G, K s) {
        Queue<K> q = new Queue<K>();
        for (int v = 0; v < G.V(); v++)
            distTo[v] = INFINITY;
        distTo[G.getVertices().getIndex(s)] = 0;
        marked[G.getVertices().getIndex(s)] = true;
        q.enqueue(s);

        while (!q.isEmpty()) {
            K v = q.dequeue();
            for (K w : G.adj(v)) {
                if (!marked[G.getVertices().getIndex(w)]) {
                    edgeTo[G.getVertices().getIndex(w)] = v;
                    distTo[G.getVertices().hash(w)] = distTo[G.getVertices().hash(v)] + 1;
                    marked[G.getVertices().hash(w)] = true;
                    q.enqueue(w);
                }
            }
        }
    ...
    private boolean check(Grafo G, int s) {
